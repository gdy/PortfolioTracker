...
| | |
|---|---|
| **11 data sources, automatic failover** | Picks whichever source covers the most symbols, then backfills individual missing fields from the others. Seven are keyless *and* direct-CORS — including stocks, spot metals and Alpaca's crypto snapshots — so the no-key path never depends on the proxy fleet for a stock or crypto price. |
...
**Optional free API keys** (Settings panel) unlock the rest: [Alpaca](https://app.alpaca.markets/signup) adds a real-time trade stream for your 30 largest stock positions, real bid/ask, and a delayed-SIP fallback that covers names IEX alone misses (Alpaca's real-time crypto needs no key and runs for everyone); [FinnHub](https://finnhub.io/register) adds a second stream plus P/E, EPS, beta, dividends, earnings dates, and analyst ratings; an [FMP](https://site.financialmodelingprep.com/register) key adds a third quote fallback, but only keys issued before September 2025 (FMP retired the endpoint this app uses for newer free plans).
...
flowchart TD
    ...
    T --> W2["Wave 2 · full fan-out<br/>FinnHub · Alpaca · FMP · Yahoo chart via proxy<br/>(only for rows nothing direct could price or name)"]
...
- **Eleven sources run concurrently.** The one covering the most requested symbols becomes primary; the rest are merged in field-by-field to fill gaps (a source might have the price but not the 52-week range).
- **Yahoo Finance has no CORS headers**, so it goes through a public CORS proxy — and only for what the direct sources can't cover: a row StockAnalysis already priced and named makes no proxy call at all, and a name-only lookup that fails backs off for 2 minutes. In steady state the only proxied requests are commodity futures, which have no direct source, and a year of daily closes per stock for YTD/6M/1Y (once per session). A first-time visitor who hasn't added anything makes no proxy request at all, and adding a stock that StockAnalysis can price makes none either — the proxied chart only runs if every direct source misses. The pool code still races multiple proxies in parallel when there are several, but after the September 2026 audit only one free proxy works from a deployed site (see the graveyard).
- **Seven sources are keyless *and* direct-CORS** — StockAnalysis for equities and ETFs; CoinGecko, Coinbase, Binance.US, Kraken and Alpaca's crypto snapshots for crypto (Alpaca's crypto endpoint answers without a key and sends CORS headers, so it runs for every visitor); and gold-api.com for spot metals. This is the structural change that matters most: a no-key portfolio of stocks and crypto now gets every price, day change, and 52-week range without touching a proxy at all. Yahoo is still keyless, but it is the only keyless source that needs the proxy fleet, and it is now used for what nothing else provides keylessly (historical performance, futures, company names) rather than for prices. Fundamentals come from FinnHub only: Yahoo's fundamentals endpoint is closed to anonymous callers.
...
- **Analyst ratings** — Strong Buy → Strong Sell consensus from FinnHub (free key), color-tinted by score. Click for a popover with the 1–5 score on a green-to-red scale and a stacked breakdown of analyst counts.
...
| **Yahoo Finance** | — | via CORS proxies | v8 chart only: quotes and company names for rows nothing direct covers, after-hours candles, historical performance, and commodity futures |
| **FinnHub** | free | 60/min + WebSocket (app budgets itself to 50/min) | WebSocket streaming (stocks), quotes, profiles, P/E, EPS, beta, dividends, earnings, analyst ratings |
...
- **Yahoo v7 batch and the crumb are permanently closed, not temporarily broken.** The batch endpoint returns `401 Unauthorized` for everyone, and `/v1/test/getcrumb` returns `401 Invalid Cookie` on both CDN hosts through any proxy. The crumb requires a Yahoo consent cookie issued by `fc.yahoo.com`, and a stateless passthrough proxy makes each request independently — it cannot carry that cookie, and the browser cannot hold a cross-origin cookie for Yahoo. **Fixing this requires a stateful backend, which is the one thing this project rules out.** So the app no longer calls the crumb, v7, v6 or `quoteSummary` at all; the only Yahoo endpoint it uses is the per-symbol **v8 chart**. They used to get one try per session, latched off on the first `401`/`404` — but measured through the proxy from the production origin they never answer `401`/`404`. They come back as `500` or `522` after 14–20 s, so the latch never fired, and every one of them was retried all session while holding a connection to the only proxy host.
- **FinnHub budget** — every FinnHub REST call draws from one sliding one-minute window of 50 calls (the free plan allows 60). At a 2 s refresh interval, per-symbol `/quote` polling for stocks that weren't streaming used to run to several hundred calls a minute. Over budget, the least-recently-quoted symbols go first next time and prices keep coming from the other sources.
...
- **Fast new-ticker fetch** — adding a position races StockAnalysis, FinnHub, Alpaca and (for gold) a Coinbase PAXG lookup in parallel, applying whichever returns a valid price first. StockAnalysis usually wins outright at ~30 ms warm. The proxied Yahoo chart only runs for a stock if every direct racer misses; commodities race it from the start, since most have no direct source. A background warmer then fires the four FinnHub fundamentals endpoints without the inter-batch delays, so P/E and analyst rating land in ~1 s rather than at the next refresh.
...
- **Yahoo depends on public CORS proxies**, which are the least reliable link in the chain. This used to take stock prices down with it for no-key users; StockAnalysis now covers keyless equity prices without a proxy, so a proxy-fleet outage degrades **historical performance, commodity futures, and first-load company names** rather than the price column. None of those has a keyless direct-CORS source — adding a free FinnHub or Alpaca key is still the single biggest reliability upgrade available.
...
Response *values* are treated the same way. Most numeric fields go through formatters built on `Intl.NumberFormat`, which can only emit digits. The exceptions were analyst counts and scores, concatenated directly: Yahoo's `quoteSummary` reader (since removed along with the endpoint) passed `raw` values through without a type check, so a proxy returning `numberOfAnalystOpinions: {raw: "<img onerror=…>"}` reached `innerHTML`. The render sites now accept only finite numbers there. The free-text date fields a payload controls verbatim — ex-dividend and next-earnings — are escaped and length-capped at the render site. They were previously interpolated raw into the table, which a compromised source could have turned into script execution; the CSP does not help there, since `script-src` has to allow `'unsafe-inline'` for the app's own handlers. Company names are only taken from a proxied payload if they are strings, and the attribute escaper coerces its input, because a numeric name used to throw inside the table renderer — and since names are cached, it threw again on every reload.
...
Not investment advice. Every price, fundamental and analyst rating shown comes from free third-party sources, several of them unofficial, and can be delayed, incomplete or wrong. Check with your broker before acting on anything the app shows. Provided as-is under the MIT license.
