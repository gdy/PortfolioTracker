...
**CoinGecko** — local cache with a TTL that scales to the portfolio size. With ≤ 5 crypto positions the cache turns over as fast as the user's chosen refresh interval (down to a 3 s hard floor — at most 20 calls/min, ~33% under CoinGecko's free 30 req/min limit) so small portfolios refresh on every tick instead of waiting 30 s. Even at the new 2 s refresh setting the floor pins CoinGecko to 1 call per 3 s. `/coins/markets` is one batched call regardless of token count, so token count doesn't change the call rate. Larger portfolios stay on the conservative 30 s TTL. `/coins/markets` is used for prices + 24h data; `/coins/{id}/market_chart?days=365` is used once per crypto symbol for 52-week range and performance metrics (with 1.5 s between requests).
