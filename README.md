...
- **A stream that goes quiet on one symbol hands it over.** Alpaca's free stream carries IEX only — a single venue with a few percent of consolidated volume — so a thinly-traded name can sit silent there while printing elsewhere. If a symbol gets no Alpaca trade for 3 minutes **while other symbols on that same socket are printing**, it moves to FinnHub, whose free stream isn't tied to one exchange. That second condition is the whole trick: it separates “this symbol is quiet” from “the market is quiet”, and it means a single-symbol subscription can never exile itself. Only during the regular session, only when FinnHub is connected with room to take it, and the list resets when Alpaca reconnects — so a borderline symbol can't ping-pong between streams all day.
